...
{
    /// <summary>
    /// Custom editor for RectMask2DAdvanced with visual preview and quick actions
    /// </summary>
    [CustomEditor(typeof(RectMask2DAdvanced))]
    public class RectMask2DAdvancedEditor : Editor
    {
        private SerializedProperty m_EdgeSoftness;
        private SerializedProperty m_AutoManageRenderers;

        private RectMask2DAdvanced m_Target;

        private void OnEnable()
        {
            m_Target = target as RectMask2DAdvanced;
            m_EdgeSoftness = serializedObject.FindProperty("m_EdgeSoftness");
            m_AutoManageRenderers = serializedObject.FindProperty("m_AutoManageRenderers");
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                ...
                "All child UI elements will automatically receive softness.",
                MessageType.Info
            );

            EditorGUILayout.Space();

            // Auto manage renderers toggle
            EditorGUILayout.PropertyField(m_AutoManageRenderers, new GUIContent("Auto Manage Renderers"));

            if (!m_AutoManageRenderers.boolValue)
            {
                EditorGUILayout.HelpBox(
                    "Auto management is disabled. You'll need to manually add AdvancedSoftnessRenderer to child UI elements.",
                    MessageType.Warning
                );
            }
            ...
            EditorGUILayout.LabelField("Edge Softness", EditorStyles.boldLabel);
            Vector4 softness = m_EdgeSoftness.vector4Value;

            EditorGUILayout.BeginHorizontal();
            softness.x = Mathf.Max(0, EditorGUILayout.FloatField("Left", softness.x));
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            softness.z = Mathf.Max(0, EditorGUILayout.FloatField("Right", softness.z));
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            softness.w = Mathf.Max(0, EditorGUILayout.FloatField("Top", softness.w));
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            softness.y = Mathf.Max(0, EditorGUILayout.FloatField("Bottom", softness.y));
            EditorGUILayout.EndHorizontal();

            m_EdgeSoftness.vector4Value = softness;
        ...
        {
            m_EdgeSoftness.vector4Value = new Vector4(value, value, value, value);
            serializedObject.ApplyModifiedProperties();
        }

        private void SetHorizontalOnly(float value)
        {
            m_EdgeSoftness.vector4Value = new Vector4(value, 0f, value, 0f);
            serializedObject.ApplyModifiedProperties();
        }

        private void SetVerticalOnly(float value)
        {
            m_EdgeSoftness.vector4Value = new Vector4(0f, value, 0f, value);
            serializedObject.ApplyModifiedProperties();
        }

        private void SetTopOnly(float value)
        {
            m_EdgeSoftness.vector4Value = new Vector4(0f, 0f, 0f, value);
            serializedObject.ApplyModifiedProperties();
        }

        private void SetBottomOnly(float value)
        {
            m_EdgeSoftness.vector4Value = new Vector4(0f, value, 0f, 0f);
            serializedObject.ApplyModifiedProperties();
        }

        private void SetLeftOnly(float value)
        {
            m_EdgeSoftness.vector4Value = new Vector4(value, 0f, 0f, 0f);
            serializedObject.ApplyModifiedProperties();
        }

        private void SetRightOnly(float value)
        {
            m_EdgeSoftness.vector4Value = new Vector4(0f, 0f, value, 0f);
            serializedObject.ApplyModifiedProperties();
        }
        ...
        private void DrawSoftnessPreview()
        {
            ...
            float leftSoft = Mathf.Min(softness.x, availableWidth);
            float rightSoft = Mathf.Min(softness.z, availableWidth);
            float bottomSoft = Mathf.Min(softness.y, availableHeight);
            float topSoft = Mathf.Min(softness.w, availableHeight);
            ...
            //     $"Left: {softness.x:F1} | Right: {softness.z:F1} | Top: {softness.w:F1} | Bottom: {softness.y:F1}",
